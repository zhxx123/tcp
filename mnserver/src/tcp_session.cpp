...
namespace mnserver
{
    namespace session_stuff
    {
        ...
        {
            // std::cout<<"直接发送消息列表到SessionHandler SendMessageListDirectly\n";
            ...
            if (handler_ptr != nullptr)
            {
                ...
                for (size_t i = 0; i < messages_received.size(); ++i)
                {
                    handler_ptr->onMessage(messages_received[i]);
                }
                messages_received.clear();
            }
            else{
                // std::cout<<"handler_ptr is nullptr.\n";
            }
        }
    }
    ...
    void TCPSession::handle_read(asio::error_code error_code, size_t bytes_transferred)
    {
        ...
        // for(int i=0;i<buffer_receiving_.size();i++) std::cout<<buffer_receiving_[i];
        size_t bytes_read = msg_filter_->read(buffer_receiving_, messages_received_);
        assert(bytes_read == bytes_transferred);
        // std::cout<<"\n1 read: "<<buffer_receiving_.size()<<" "<<messages_received_.size()<<" "<<bytes_read<<" "<<bytes_transferred<<std::endl;
        ...
        if (wanna_post)
        {
            // std::cout<<"wana_post: "<<(io_thread_->get_id() == io_thread_->get_thread_manager().get_main_thread()->get_id())<<std::endl;
        ...
        // std::cout<<"2 read: "<<buffer_receiving_.size()<<" "<<bytes_wanna_read<<" "<<bytes_read<<" "<<bytes_transferred<<std::endl;
    }

    // 处理写
    void TCPSession::hanlde_write(asio::error_code error_code, size_t bytes_transferred)
    {
        // std::cout<<"handler_write\n";
        --num_write_handlers_;
