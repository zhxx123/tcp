...
namespace mnserver
{
    ...
    void NetMessage::retrieve(size_t size)
    {
        ...
        // std::cout<<readable()<<" "<<size<<" "<<(readable() > size?"readable()>size":"null")<<std::endl;
        if (readable() > size)
        {
            reader_pos_ += size;
        }
    ...
    std::string NetMessage::read_string()
    {
        ...
        // std::cout<<"read_string: "<<lenght<<" "<<readable() <<" "<<(readable() >= lenght)<<std::endl;
        assert(readable() >= lenght);
        if(lenght>readable()) lenght=readable();
        std::string value;
        if (lenght > 0)
        {
            value.resize(lenght);
        ...
        // for(int i=0;i<value.size();i++) std::cout<<value[i]<<std::endl;
        // std::cout<<"\n";
        return value;
    }
    ...
    std::string NetMessage::read_lenght_and_string()
    {
        ...
        if (lenght > 0)
        {
            // std::cout<<"read_lenght_and_string: "<< lenght <<std::endl;
            value.resize(lenght);
            memcpy(const_cast<char*>(value.data()), data(), lenght);
            retrieve(lenght);
        }
